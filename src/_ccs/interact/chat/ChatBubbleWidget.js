 ...
 *
 */
_ccs.ChatBubbleWidget = ccui.Widget.extend({
    ...
    /**
     * _ccs.ChatBubbleWidget constructor
     * @constructor
     */
    ctor: function () {
    ...
    _initCCSChildren: function () {
        ...
        this._layoutBubble.setTouchEnabled(false);
        ...
        ck.UIUtils.setPositionPercent(this._layoutContent, -0.4, 0.22);
        ...
        ///// # _richTxtContent
        this._richTxtContent = new ck.CustomRichTextExtend(cc.size(180, 80));
        this._richTxtContent.setName("_richTxtContent");
        this._layoutContent.addChild(this._richTxtContent);
        this._richTxtContent.setCascadeOpacityEnabled(true);
        this._richTxtContent.setCascadeColorEnabled(true);
        this._richTxtContent.setAnchorPoint(cc.p(0.5, 0.5));
        ck.UIUtils.setPositionPercent(this._richTxtContent, 0.5, 0.53);
        this._richTxtContent.setVisible(false);
        this._richTxtContent.setDefaultColor(cc.color(26, 26, 26, 255));
        let _richTxtContentLayoutComponent = ccui.LayoutComponent.bindLayoutComponent(this._richTxtContent);
        _richTxtContentLayoutComponent.setPositionPercentXEnabled(true);
        _richTxtContentLayoutComponent.setPositionPercentX(0.5);
        _richTxtContentLayoutComponent.setPositionPercentYEnabled(true);
        _richTxtContentLayoutComponent.setPositionPercentY(0.53);
        _richTxtContentLayoutComponent.setPercentWidthEnabled(true);
        _richTxtContentLayoutComponent.setPercentWidth(0.9);
        _richTxtContentLayoutComponent.setPercentHeightEnabled(true);
        _richTxtContentLayoutComponent.setPercentHeight(0.8);
        this._richTxtContent.setTouchEnabled(false);
        this._richTxtContent.setDefaultFont("fonts/BebasNeueProBold.ttf");
        this._richTxtContent.setDefaultSize(35);
        this._richTxtContent.setDefaultAlignHorizontal(2);
        this._richTxtContent.setDefaultAlignVertical(5);
        ...
        this._layoutArrow.setTouchEnabled(false);
    ...
    playCCSAnimOpen: function (delay = 0, callback = null, repeat = 1, speed = 1) {
        ...
        let _layoutBubbleAction = cc.sequence(
            cc.spawn(
                ...
                cc.rotateBy(30 / (60 * speed), 15, 15).easing(cc.easeBackOut())
            ),
            cc.callFunc(function () {
                if (repeat === 1) return;
                this._layoutBubble.setScale(0.0001, 0.0001);
                this._layoutBubble.setRotation(-15);
            }.bind(this))
        );
        if (repeat <= 0) {
    ...
    playCCSAnimClose: function (delay = 0, callback = null, repeat = 1, speed = 1) {
        ...
        let _layoutBubbleAction = cc.sequence(
            cc.spawn(
                ...
                cc.rotateBy(30 / (60 * speed), -15, -15).easing(cc.easeBackIn())
            ),
            cc.callFunc(function () {
                if (repeat === 1) return;
                this._layoutBubble.setScale(1, 1);
                this._layoutBubble.setRotation(0);
            }.bind(this))
        );
        if (repeat <= 0) {
            _layoutBubbleAction = _layoutBubbleAction.repeatForever();
...
/**
 * Create ChatBubbleWidget
 * @returns {_ccs.ChatBubbleWidget}
 */
_ccs.ChatBubbleWidget.create = function () {
    return new _ccs.ChatBubbleWidget();
};
